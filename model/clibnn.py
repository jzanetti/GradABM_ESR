...
from model.model_utils import DecodeSeq, EmbedAttenSeq
...
MIN_VAL_PARAMS = {
    ...
    "seirm": [0.0, 0.0, 0.0, 0.0, 0.01],  # beta, alpha, gamma, mu, initial_infections_percentage
...
class CalibNN(nn.Module):
    def __init__(
        ...
        self.out_layer = [
            ...
            nn.ReLU(),
            nn.Linear(in_features=out_layer_width // 2, out_features=para_dim),
            # nn.Linear(in_features=out_layer_width // 2, out_features=out_dim),
            # nn.Linear(in_features=out_layer_width // 2, out_features=X_train_dim),
        ]
