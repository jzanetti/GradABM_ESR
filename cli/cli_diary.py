...
from process.diary.diary import create_diary_remote, read_synthetic_pop
...
def setup_parser():
    ...
    return parser.parse_args(
        [
            "--synthetic_pop_path",
            "/tmp/syspop_test/Wellington/syspop_base.csv",
            "--workdir",
            "/tmp/gradabm_esr/Wellington",
            "--ncpu",
            "2",
        ]
    )
...
def main(workdir: str, synthetic_pop_path: str, ncpu: int):
    ...
    for i, proc_syspop_data in enumerate(syspop_data_partitions):
        print_log = i == 0
        outputs.append(
            create_diary_remote.remote(proc_syspop_data, ncpu, print_log=print_log)
        )
